...
const Vehicles = () => {
    ...
    const handleClick = async (event, vehicleName) => {
        ...
        await fetch('http://35.2.2.224:5000/api/v1/settings/vehicles/', {method: 'delete', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({'vehicleName': vehicleName})})
            .catch(error => console.log(error));
    };
    
    useEffect(() => {
        fetch('http://35.2.2.224:5000/api/v1/settings/vehicles/', {method: 'get', headers: {'Content-Type': 'application/json'}})
        ...
        <Box style={{display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '90vw'}}>
            <Box style={{display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', width: '80vw', minHeight: '80vh'}}>
                ...
                <Box style={{display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
                    ...
                    <form action='http://35.2.2.224:5000/api/v1/settings/vehicles/?target=/settings/vehicles' method='post' encType='multipart/form-data'>
